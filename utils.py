...
class gradientComparison():
    """meant to be included as part of learningStats()


    """
    def __init__(self):
        #these should only save for a single epoch
        self.bp_grads = []
        self.nbp_grads= []
        #these should hold angle distances for over entire epochs
        self.angle_distance = []
    ...
    def angle_between(self, A, B):
        ...
        # print(normalized_flat_A)
        # print(normalized_flat_B)
        angle = (180.0 / np.pi) * np.arccos(np.clip(np.dot(normalized_flat_A, normalized_flat_B), -1.0, 1.0))
        # print(angle)
        return angle

    def clear_grad_arrays(self):
        #compile gradients properly
        angle_list = np.empty(len(self.nbp_grads[0]))
        for layer in range(len(self.nbp_grads[0])):
            nbp_gradlist = []
            bp_gradlist = []
            for batch in range(len(self.nbp_grads)):
                try:
                    nbp_gradlist.append(self.nbp_grads[batch][layer])
                    bp_gradlist.append(self.bp_grads[batch][layer])
                except:
                    print(len(self.nbp_grads), len(self.nbp_grads[0]))
                    print(len(self.bp_grads), len(self.bp_grads[0]))

            nbp_grads = np.concatenate(nbp_gradlist)
            bp_grads = np.concatenate(bp_gradlist)
            angle = self.angle_between(nbp_grads, bp_grads)
            angle_list[layer] = angle
        self.angle_distance.append(angle_list)
        self.bp_grads = []
        self.nbp_grads = []


    def updateBP(self, epoch, batchid, update):
        if (batchid == 0 and epoch != 0):
            self.clear_grad_arrays()
        self.bp_grads.append(update)

    def updateNBP(self, epoch, batchid, update):
        # print(update)
        # if (batchid == 0 and epoch != 0):
        #     self.clear_grad_arrays()
            #compile gradients properly
        ...
        self.nbp_grads.append(update)


    def finalize_values(self):
        self.clear_grad_arrays()
        self.finarray = np.array(self.angle_distance)


    def print_angular_simple(self):
        # self.clear_grad_arrays()

        # self.finarray = np.concatenate(self.angle_distance, dim=0)
        print(self.finarray)
        # print("calculating angular distance:")
        # print("length of init vector:", len(self.bp_grads))
        # print("number of estimated batch peices: ", len(self.bp_grads[0]))
        # print("shoulod be number of layers: ", len(self.bp_grads[0][0]))
        # print("should be shape of layer 0 grads: ", self.bp_grads[0][0][0].shape)
        # for layer in range(len(self.bp_grads[0][0])):
        #     print("--------------LAYER ", layer, "---------------------")
        #     for epoch in range(len(self.bp_grads)):
        #         p1 = self.bp_grads[epoch][0][layer]
        #         # p1b = self.bp_grads[i][0][1].cpu().numpy()
        #         # print(p1)
        #         # print(p1b)
        #         p2 = self.nbp_grads[epoch][0][layer]
        #         # print(p2)
        #         angle = self.angle_between(p1, p2)
        #         print("epoch ", epoch, ": ", angle)
                # exit()

    def graph_angular_simple(self, filename=''):

        if (filename== ''):
            filename = './postprocessing/graphs/fc_3l_figure_'
        figurestart = 20
        for layer in range(self.finarray.shape[1]):
            filestring = filename + str(layer) + ".png"
            print(layer)
            plt.figure(figurestart + layer)
            plt.plot(self.finarray[:,layer])
            plt.title("layer " + str(layer) + " feedback (kp vs. bp)")
            plt.xlabel('epoch')
            plt.ylabel('degrees off from bp')
            try:
                plt.savefig(filestring)
            except:
                print("savefig didn't work")
...
class learningStats():
    ...
    def __init__(self, gradients=False):
        ...
        self.alt = learningStat()
        if (gradients):
            self.keep_gradients = True
            self.gradients = gradientComparison()
    ...
    def updateBPLogs(self, epoch, batch_id, bp_grads):
        self.gradients.updateBP(epoch, batch_id, bp_grads)

    def updateNBPLogs(self, epoch, batch_id, nbp_grads):
        self.gradients.updateNBP(epoch, batch_id, nbp_grads)
...
class EarlyStopping:
    ...
    def __init__(self, patience=50, verbose=False, delta=0, file_prepend = ""):
        """
        Args:
    ...
    def save_checkpoint(self, network, val, epoch, filename=""):
