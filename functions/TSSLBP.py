...
class PSP_spike_large_batch_ni(torch.autograd.Function): 
    @staticmethod
    def forward(ctx, inputs, network_config, layer_config):
        shape = inputs.shape
        n_steps = network_config['n_steps']
...
class PSP_spike_long_time_ni(torch.autograd.Function):  
    @staticmethod
    def forward(ctx, inputs, network_config, layer_config):
        shape = inputs.shape
        n_steps = network_config['n_steps']
    ...
    def backward(ctx, grad_delta):
        ...
        for t in range(n_steps):
            ...
            grad_a[..., t] = torch.sum(partial_a_inter[..., t:n_steps]*grad_delta[..., t:n_steps], dim=4) 
