...
class PSP_spike_large_batch(torch.autograd.Function): 
    @staticmethod
    def forward(ctx, inputs, network_config, layer_config, backward_matrix):
        shape = inputs.shape
        n_steps = network_config['n_steps']
        ...
        ctx.save_for_backward(mem_updates, outputs, mems, delta_refs, backward_matrix, torch.tensor([threshold]))
        return syns_posts
    ...
    def backward(ctx, grad_delta):
        ...
        # out: grad: delta(l-1)
        (delta_u, outputs, u, delta_refs, backward_matrix, others) = ctx.saved_tensors
        start_time = time()
        feedback = glv.dfa_feedback
        shape = outputs.shape
        outputshape = backward_matrix.shape[-1]
        if (shape[2] == 1):
            neuron_feedback = torch.einsum('ab, cbdef -> cadef', backward_matrix, feedback)
        else:
            neuron_feedback = torch.einsum('abcd, edfgh -> eabch', backward_matrix, feedback)
        ...
        #reshape neuron feedback here
        re_shape = (shape[0], shape[1], shape[2], shape[3], shape[4])
        neuron_feedback = neuron_feedback.reshape(re_shape)
        neuron_feedback = (neuron_feedback.expand(shape))
        for i in range(int(shape[0]/mini_batch)):
            ...
            grad_a[i*mini_batch:(i+1)*mini_batch, ...] = torch.einsum('...ij, ...j -> ...i', partial_a_all, neuron_feedback[i*mini_batch:(i+1)*mini_batch, ...])
...
class PSP_spike_long_time(torch.autograd.Function):  
    ...
    def backward(ctx, grad_delta):
        ...
        shape = outputs.shape
        outputshape = backward_matrix.shape[-1]
        # print(neuron_feedback.shape)
        # print(backward_matrix.shape)
        ...
        for t in range(n_steps):
            ...
            grad_a[..., t] = torch.sum(partial_a_all[..., t:n_steps]*neuron_feedback[..., t:n_steps], dim=4) 
